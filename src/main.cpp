...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // desenho dos 16 coelhos e ovos
        for (int i = 0; i < 16; i++)
        {
            RenderBunnySet(g_OrbitOffsets[i] * g_BunnySpace, g_WavePhases[i]);
        }
...
void RenderBunnySet(float orbit_offset, float wave_phase)
{
    ...
    // COELHO
    if (wave_phase == 0.0f) {
        model = Matrix_Rotate_Y(time * 0.5f + orbit_offset) // rotação da órbita do coelho
